...
public class CollectionFilter extends RandomPackageAPI {
    ...
    public void enable() {
        ...
        for(int i = 0; i < collectionchestgui.getSize(); i++) {
            if(config.get("gui." + i) != null) {
                ...
                picksup.put(i, um);
            } else cci.setItem(i, background.copy());
        }
    ...
    @Listener
    private void entityDeathEvent(DestructEntityEvent event) {
    ...
    @Listener
    private void playerInteractEvent(InteractEvent event) {
        ...
        if(!event.isCancelled() && b.getType().equals(collectionchest.getType())) {
            ...
            if(cc != null) {
                ...
                if(config.getBoolean("collection chests.chest.keeps meta")) {
                    ...
                    final ItemStack is = collectionchest.copy();
    ...
    public ItemStack getCollectionChest(String filter) {
        ...
        item = collectionchest.copy(); itemMeta = item.getItemMeta(); lore.clear();
        if(itemMeta.hasLore()) {
