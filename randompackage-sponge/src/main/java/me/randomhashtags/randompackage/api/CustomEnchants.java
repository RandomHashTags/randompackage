...
public class CustomEnchants extends CustomEnchantUtils {
        ...
        if(RP.getBoolean("custom enchants.enchantment orbs")) {
            ...
            for(String A : yml.getConfigurationSection("orbs").getKeys(false)) {
                ...
                for(int k = starting; k <= yml.getInt("orbs." + A + ".final max slots"); k += increment) {
                    ...
                    item = item.copy(); itemMeta = item.getItemMeta(); lore.clear();
    ...
    public ItemStack getRevealedItem(CustomEnchant enchant, int level, int success, int destroy, boolean showEnchantType, boolean showOtherLore) {
        ...
        item = rarity.getRevealedItem().copy(); itemMeta = item.getItemMeta(); lore.clear();
        ...
        if(!event.isCancelled() && top.getHolder() == player) {
            ...
            if(t.equals(tinkerer.getTitle())) {
                ...
                if(r >= 4 && r <= 8
                    ...
                    if(xp != 0) item = givedpitem.getXPBottle(xp, "Tinkerer").copy();
                } else {
            ...
            if(title.equals(tinkerer.getTitle())) {
                ...
                if(r < size) {
                    if(r == 3 || r == 5) {
                        ...
                        item = alchemistpreview.copy();
                        if(!top.getItem(13).equals(item)) top.setItem(13, item);
                        item = alchemistexchange.copy();
                        ...
                        if(!e.isCancelled()) {
                            ...
                            item = top.getItem(13).copy(); itemMeta = item.getItemMeta();
                    ...
                    if(suCCess != null) {
                        ...
                        if(F == 5 && !top.getItem(3).getItemMeta().getDisplayName().equals(d)
                            ...
                            if(suCCess.equals("dust")) {
                                ...
                                if(u != null) {
                                    item = top.getItem(slot).copy(); itemMeta = item.getItemMeta(); lore.clear();
                                    ...
                                    for(int i = 0; i < itemMeta.getLore().size(); i++) {
                                        if(getRemainingInt(itemMeta.getLore().get(i)) != -1 && !did) {
                                            ...
                                            item = item.copy(); itemMeta = item.getItemMeta();
                                            for(String s : itemMeta.getLore()) {
                                ...
                                item = getRevealedItem(enchant, level + 1, success, destroy, true, true).copy(); itemMeta = item.getItemMeta();
                            }
                            upgrade = true;
                        }
                        if(upgrade) {
                            ...
                            top.setItem(13, item);
                            item = alchemistaccept.copy(); itemMeta = item.getItemMeta(); lore.clear();
                            for(String string : itemMeta.getLore()) {
                ...
                if(enchantercost.containsKey(r)) {
                    ...
                    item = enchanterpurchase.get(r).copy();
                    List<String> me = null;
                    final int totalxp = getTotalExperience(player);
                ...
                if(!ev.isCancelled() && isOnCorrectItem(enchant, current)) {
                    boolean apply = false;
                    item = current.copy(); itemMeta = item.getItemMeta(); lore.clear();
        ...
        if(is != null && enchants.size() > 0) {
            ...
            for(int f = 1; f <= 5; f++) {
                ...
                if(a.contains(r)) {
                    ...
                    return getRevealedItem(enchant, enchantlevel, successP, 100, true, true).copy();
