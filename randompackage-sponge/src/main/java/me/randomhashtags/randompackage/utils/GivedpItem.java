...
public class GivedpItem extends RandomPackageAPI {
        ...
        if(input.startsWith("banknote:")) {
            ...
            if(mcmmoIsEnabled) {
                ...
                if(m.isEnabled) {
                    ...
                    final ItemStack i = lvl ? items.get("mcmmolevelvoucher").copy() : xp ? items.get("mcmmoxpvoucher").copy() : items.get("mcmmocreditvoucher").copy();
            ...
            return customitems.get(Q).copy();
        } else if(items != null && items.containsKey(input)) {
            return items.get(input).copy();
        }
        return null;
    }

    public ItemStack getBanknote(double value, String signer) {
        item = items.get("banknote").copy(); itemMeta = item.getItemMeta(); lore.clear();
        for(String s : itemMeta.getLore()) {
    ...
    public ItemStack getXPBottle(int value, String enchanter) {
        item = items.get("xpbottle").copy(); itemMeta = item.getItemMeta(); lore.clear();
        for(String s : itemMeta.getLore()) {
    ...
    private void playerChatEvent(AsyncPlayerChatEvent event) {
        ...
        if(itemnametag.contains(player)) {
            ...
            if(item == null || item.getType().equals(Material.AIR)) {
                ...
                giveItem(player, items.get("itemnametag").copy());
            ...
            if(item == null || item.getType().equals(Material.AIR)) {
                ...
                giveItem(player, items.get("itemlorecrystal").copy());
            } else if(item.getType().name().endsWith("BOW") || item.getType().name().endsWith("_AXE") || item.getType().name().endsWith("SWORD") || item.getType().name().endsWith("HELMET") || item.getType().name().endsWith("CHESTPLATE") || item.getType().name().endsWith("LEGGINGS") || item.getType().name().endsWith("BOOTS")) {
                itemMeta = item.getItemMeta(); lore.clear();
                boolean did = false;
                if(itemMeta.hasLore()) {
