...
public class CustomEnchantUtils extends RandomPackageAPI {
    ...
    public void w(CustomEnchantProcEvent ev, Event event, CustomEnchant enchant, List<Living> recipients, String a, String attribute, int b, Player P) {
        try {
    ...
    private void executeAttributes(CustomEnchantProcEvent ev, Event event, CustomEnchant enchant, List<Living> recipients, String a, String attribute, int b, Player P) {
        ...
        final HashMap<String, Living> recipientss = getRecipients(event, P);
        ...
        for(int i = 1; i <= 6; i++) {
            if(a.contains("maxHealthOf(") || a.contains("healthOf(")) {
                ...
                if(isPVAny) {
                    ...
                    final Living damager = e.damager, victim = e.victim;
        ...
        if(a.contains("direction")) {
            ...
            final Living recip = getRecipient(event, r);
            if(recip != null) {
        ...
        if(a.contains("nearby{") || a.contains("nearbyAllies{") || a.contains("nearbyEnemies{")) {
            ...
            final List<Living> r = new ArrayList<>();
            final Living who = event instanceof PluginEnableEvent ? P : getRecipient(event, e.split(":")[1]), k = getRecipient(event, e.split(":")[0]);
            if(who != null) {
                for(Entity en : who.getNearbyEntities(oldevaluate(e.split(":")[2]), oldevaluate(e.split(":")[3]), oldevaluate(e.split(":")[4]))) {
                    if(en instanceof Living && en instanceof Damageable && (k == null || k != null && !en.equals(k)))
        ...
        if(a.toLowerCase().startsWith("addpotioneffect{")) {
            ...
            for(Living l : recipients) setVelocity(l, knockback);
            ...
            final Living receiver = getRecipient(event, aa[0]), target = getRecipient(event, aa[1]);
    ...
    private void doIf(CustomEnchantProcEvent ev, Event event, CustomEnchant enchant, int level, String a, String attribute, int b, String input, Player P) {
        ...
        for(Living l : getRecipients(event, input, P)) {
    ...
    private boolean doVariable(CustomEnchantProcEvent e, Event event, CustomEnchant enchant, Living entity, String input) {
        if(input.startsWith("isHolding("))      return isHolding(entity, input.split("\\(")[1].split("\\)")[0]);
            ...
            if(event instanceof PvAnyEvent) {
                final Living victim = ((PvAnyEvent) event).victim;
            ...
            final List<Living> recipients = getRecipients(event, input.split("distanceBetween\\(")[1].split("\\)")[0], null);
    ...
    public HashMap<String, Living> getRecipients(Event event, Player p) {
        final HashMap<String, Living> recipients = new HashMap<>();
        if(event instanceof CustomEnchantEntityDamageByEntityEvent) {
            ...
            if(e.damager instanceof Living) recipients.put("DAMAGER", (Living) e.damager);
            ...
            recipients.put("VICTIM", (Living) ((EntityDamageEvent) event).getEntity());
        } else if(event instanceof PluginEnableEvent) {
            recipients.put("PLAYER", p);
        }
        return recipients;
    }
    public List<Living> getRecipients(Event event, String input, Player p) {
        ArrayList<Living> recipients = new ArrayList<>();
        if(event instanceof CustomEnchantEntityDamageByEntityEvent) {
            final CustomEnchantEntityDamageByEntityEvent e = (CustomEnchantEntityDamageByEntityEvent) event;
            if(input.toLowerCase().contains("owner")) recipients.add(e.getCustomEnchantEntity().getSummoner());
            if(input.toLowerCase().contains("damager") && e.damager instanceof Living) recipients.add((Living) e.damager);
        } else if(event instanceof ArmorSetEquipEvent) {
            ...
            if(input.toLowerCase().contains("shooter") && e.getEntity().getShooter() instanceof Living) recipients.add((Living) e.getEntity().getShooter());
            final Living hit = getHitEntity(e);
            if(input.toLowerCase().contains("victim") && hit != null) recipients.add(hit);
        } else if(event instanceof PvAnyEvent) {
            final PvAnyEvent e = (PvAnyEvent) event;
            final Living v = e.victim;
            ...
            if(input.toLowerCase().contains("shooter") && pro != null && pro.getShooter() instanceof Living) recipients.add((Living) pro.getShooter());
        } else if(event instanceof isDamagedEvent) {
            ...
            recipients.add((Living) ((EntityDamageEvent) event).getEntity());
    ...
    private Location getRecipientLoc(Event event, String input) {
        if(event instanceof PlayerArmorEvent) {
            ...
            if(input.toLowerCase().contains("shooter") && e.getEntity().getShooter() instanceof Living) return ((Living) e.getEntity().getShooter()).getLocation();
        } else if(event instanceof PvAnyEvent) {
            final PvAnyEvent e = (PvAnyEvent) event;
            final Living v = e.victim;
            final Projectile pro = e.proj;
            ...
            if(input.contains("shooter") && pro != null) return ((Living) pro.getShooter()).getLocation();
        }
        return null;
    }
    private Living getRecipient(Event event, String input) {
        if(event instanceof PlayerArmorEvent) {
            ...
            final ProjectileHitEvent e = (ProjectileHitEvent) event;
            if(input.toLowerCase().contains("arrow") && e.getEntity() instanceof Arrow) return (Living) e.getEntity();
            if(input.toLowerCase().contains("shooter") && e.getEntity().getShooter() != null && e.getEntity().getShooter() instanceof Living) return (Living) e.getEntity().getShooter();
        } else if(event instanceof PvAnyEvent) {
            final PvAnyEvent e = (PvAnyEvent) event;
            final Living v = e.victim;
            final Projectile pro = e.proj;
            if(input.contains("damager")) return pro instanceof Living ? (Living) pro : e.damager;
            if(input.contains("victim") && v != null) return v;
            if(input.contains("shooter") && pro != null) return (Living) pro.getShooter();
        }
        return null;
    }
    ...
    public void depleteCombo(Player player, CustomEnchant source, double depletion) {
        if(combos.keySet().contains(player) && combos.get(player).keySet().contains(source)) {
    ...
    public void addPotionEffect(Event event, Player player, Living entity, PotionEffect potioneffect, List<String> message, CustomEnchant enchant, int level) {
        if(entity != null) {
    ...
    private void removePotionEffect(Living entity, PotionEffect potioneffect, List<String> message, CustomEnchant enchant, int level) {
        if(potioneffect != null && entity != null && entity.hasPotionEffect(potioneffect.getType())) {
            entity.removePotionEffect(potioneffect.getType());
            dopotioneffect(entity, potioneffect, message, enchant, level);
        }
    }
    private void dopotioneffect(Living entity, PotionEffect potioneffect, List<String> message, CustomEnchant enchant, int level) {
        if(message != null) {
    ...
    private void damage(Living damager, Living entity, double damage, boolean heartDmg) {
        if(entity != null) {
    ...
    private void setHealth(Living entity, double newHealth) {
        if(newHealth < 0.00) entity.setHealth(0.00);
        else entity.setHealth(newHealth);
    }
    private boolean isHolding(Living entity, String input) {
        final ItemStack i = getItemInHand(entity);
        return i != null && i.getType().name().endsWith(input.toUpperCase());
    }
    private boolean isBlocking(Living entity) { return entity instanceof Player && ((Player) entity).isBlocking(); }
    private boolean healthIsLessThanOrEqualTo(Living entity, double target) { return entity.getHealth() <= target; }
    private boolean healthIsGreaterThanOrEqualTo(Living entity, double target) { return entity.getHealth() >= target; }
    ...
    private void refillAir(Living entity, int addedAir) {
        final int r = entity.getRemainingAir(), m = entity.getMaximumAir();
        entity.setRemainingAir(r + addedAir > m ? m : r + addedAir);
    }
    private int getXP(Living entity) { return entity instanceof Player ? getTotalExperience((Player) entity) : 0; }
    private void setXP(Living entity, int value) {
        if(entity instanceof Player) {
            setTotalExperience((Player) entity, value);
        }
    }
    private void dropItem(Living entity, ItemStack is) { entity.getWorld().dropItem(entity.getLocation(), is); }
    ...
    private void sendMessage(CustomEnchant enchant, Living entity, String message) {
    ...
    private void setVelocity(Living entity, Vector vel) {
    ...
    private void freeze(Living entity, int ticks) {
        if(entity instanceof Player) {
